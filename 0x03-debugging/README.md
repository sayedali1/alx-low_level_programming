...
Fix the code in [`2-largest_number.c`](/0x03-debugging/2-largest_number.c) so that it correctly prints out the largest of three numbers, no matter the case.
...
Fix the [`print_remaining_days()`](/0x03-debugging/3-print_remaining_days.c) function so that the output works correctly for all dates and all leap years.
